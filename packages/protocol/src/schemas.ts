...
export const workspaceHomeLayoutResponseSchema = z.object({ version: z.literal(1), blocks: z.array(workspaceHomeBlockSchema), panels: z.object({ explorerWidth: z.number().int().min(280).max(440), contextWidth: z.number().int().min(220).max(440) }) }) as z.ZodType<WorkspaceHomeLayoutDto>;

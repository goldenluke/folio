...
const run = async (command: string, args: readonly string[], env: NodeJS.ProcessEnv): Promise<void> => new Promise((ok, fail) => {
  const child = spawn(command, [...args], { cwd: repository, stdio: 'inherit', env });
...
try {
  const env = { ...process.env, FOLIO_NATIVE_TARGET: 'win32-x64', FOLIO_PACKAGE_STAGE: stage };
  await run('pnpm', ['--filter', '@abnt/desktop', 'build:native:windows'], env);
  await run('pnpm', ['--filter', '@abnt/desktop', 'exec', 'tsx', 'scripts/stage-package.ts'], env);

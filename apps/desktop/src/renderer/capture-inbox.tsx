...
export function CaptureInboxDialog({ workspaceId, pendingCapture, onClose, onOpenDocument, onMessage }: {
  ...
  useEffect(() => { void window.academic.workspace.captureInbox().then((result) => { if (result.ok) setItems(result.value.items); else onMessage(result.error.message); }); }, [onMessage]);
  ...
  const forwardReference = async (item: WorkspaceCaptureInboxItemDto): Promise<void> => {
    ...
    const extracted = await window.academic.workspace.webCaptureExtract({ url: item.url });

...
export function ResearchProjectsDialog({ workspaceId, files, knowledge, activeFile, onClose }: { readonly workspaceId: string; readonly files: readonly WorkspaceFileDto[]; readonly knowledge: ProjectKnowledgeState; readonly activeFile?: WorkspaceFileDto; readonly onClose: () => void }): JSX.Element {
  ...
  useEffect(() => { if (persistedWorkspace.current === workspaceId) return; persistedWorkspace.current = workspaceId; const restored = readResearchProjects(workspaceId); setProjects(restored); setSelectedId(restored.find((project) => project.archivedAt === undefined)?.id); }, [workspaceId]);
  useEffect(() => { writeResearchProjects(workspaceId, projects); }, [projects, workspaceId]);
  ...
  const create = async (): Promise<void> => { const title = await requestText('Título do projeto acadêmico', 'TCC 2026'); if (title === null || title.trim() === '') return; const project = createResearchProject(title.trim()); setProjects((current) => { const next = [...current, project]; writeResearchProjects(workspaceId, next); return next; }); setSelectedId(project.id); setShowArchived(false); };

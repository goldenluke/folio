...
import { citationSource, editableCitationAt, type CitationDraft, type CitationEditMode } from './shell/citation-source.js';
...
function CitationDialog({ fileId, initial, replaceRange, onClose, onApply }: {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/75 p-5" role="presentation">
    <section role="dialog" aria-modal="true" aria-labelledby="citation-editor-title" className="w-full max-w-xl rounded-lg border border-slate-600 bg-slate-900 p-5 shadow-2xl">
      ...
      <select value={referenceId} onChange={(event) => setReferenceId(event.target.value)} className="mt-2 w-full rounded bg-slate-800 px-3 py-2 text-sm"><option value="">Selecione uma referência…</option>{filtered.map((reference) => <option key={reference.id} value={reference.id}>{reference.id} — {reference.formatted}</option>)}</select>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2"><input value={locator} onChange={(event) => setLocator(event.target.value)} placeholder="Locator (p. 42, cap. 3)" className="rounded bg-slate-800 px-3 py-2 text-sm" /><input value={prefix} onChange={(event) => setPrefix(event.target.value)} placeholder="Prefixo" className="rounded bg-slate-800 px-3 py-2 text-sm" /><input value={suffix} onChange={(event) => setSuffix(event.target.value)} placeholder="Sufixo" className="rounded bg-slate-800 px-3 py-2 text-sm" /></div>
      <p className="mt-3 rounded bg-slate-950 p-2 font-mono text-sm text-slate-300">{referenceId === '' ? 'Selecione uma referência' : citationSource({ referenceId, mode, ...(locator === '' ? {} : { locator }), ...(prefix === '' ? {} : { prefix }), ...(suffix === '' ? {} : { suffix }) })}</p>
      <div className="mt-5 flex justify-end gap-2"><button type="button" className="rounded bg-slate-700 px-3 py-1.5 text-sm" onClick={onClose}>Cancelar</button><button type="button" disabled={referenceId === '' || range === undefined} onClick={() => range !== undefined && onApply(range, citationSource({ referenceId, mode, ...(locator === '' ? {} : { locator }), ...(prefix === '' ? {} : { prefix }), ...(suffix === '' ? {} : { suffix }) }))} className="rounded bg-cyan-700 px-3 py-1.5 text-sm disabled:opacity-40">Aplicar</button></div>
    </section></div>;
}
...
function ReferenceHealthDialog({ onClose }: { readonly onClose: () => void }): JSX.Element {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/75 p-5"><section role="dialog" aria-modal="true" className="w-full max-w-xl rounded-lg border border-slate-600 bg-slate-900 p-5 shadow-2xl"><div className="flex"><div><h2 className="text-lg font-semibold">Saúde das referências</h2><p className="text-sm text-slate-400">Projeção do catálogo do vault e das citações indexadas.</p></div><button type="button" className="ml-auto text-xl text-slate-400" onClick={onClose}>×</button></div>{health === undefined ? <p className="py-8 text-center text-slate-400">Calculando…</p> : <><div className="mt-4 grid grid-cols-2 gap-2 text-center text-sm"><div className="rounded bg-slate-800 p-3">Referências<br /><strong>{health.total}</strong></div><div className="rounded bg-cyan-950/60 p-3">Citadas<br /><strong>{health.cited}</strong></div><div className="rounded bg-amber-950/60 p-3">Não usadas<br /><strong>{health.unused}</strong></div><div className="rounded bg-slate-800 p-3">Sem DOI<br /><strong>{health.withoutDoi}</strong></div></div><div className="mt-4"><h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Chaves citadas e não encontradas</h3>{health.missing.length === 0 ? <p className="mt-1 text-sm text-slate-300">Nenhuma.</p> : <ul className="mt-1 list-disc pl-5 font-mono text-sm text-red-300">{health.missing.map((id) => <li key={id}>{id}</li>)}</ul>}</div></>}</section></div>;
}

...
export function PdfReaderDialog({ referenceId, onClose, onOpenLiteratureNote }: { readonly referenceId: string; readonly onClose: () => void; readonly onOpenLiteratureNote: (fileId: string, path: string) => void }): JSX.Element {
  ...
  useEffect(() => {
    let cancelled = false;
    const task = timePerformance('pdf-load', async (): Promise<void> => {
  ...
  const matches = useMemo(() => query.trim() === ''
    ? []
    : timePerformanceSync('pdf-search', () => pageTexts.flatMap((text, index) => normalize(text).includes(normalize(query)) ? [index + 1] : [])),
  [pageTexts, query]);
  ...
  const createAnnotation = async (): Promise<void> => {
    ...
    const comment = (await requestText('Comentário sobre o destaque (opcional)')) ?? undefined;
    const result = await window.academic.library.createPdfAnnotation({ referenceId, page: pageNumber, quote: selectedQuote, ...(comment?.trim() === '' || comment === undefined ? {} : { comment }), ...(selectedColor === undefined ? {} : { color: selectedColor }) });

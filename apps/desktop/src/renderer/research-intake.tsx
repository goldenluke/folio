...
export function ResearchIntakeDialog({ workspaceId, initialFormat, onClose, onMessage }: { readonly workspaceId: string; readonly initialFormat?: 'bibtex' | 'ris' | 'csl-json'; readonly onClose: () => void; readonly onMessage: (message: string) => void }): JSX.Element {
  ...
  const [reviewingIdentifiers, setReviewingIdentifiers] = useState(false);
  const [message, setMessage] = useState<string>();
  const pendingPdfs = useRef(new Map<string, PendingPdf>());
  const input = useRef<HTMLInputElement>(null);

  useEffect(() => { setItems(loadInbox(workspaceId)); }, [workspaceId]);
  useEffect(() => { if (initialFormat !== undefined) setFormat(initialFormat); }, [initialFormat]);
  ...
  const resolveIdentifier = async (): Promise<void> => {
    if (reviewingIdentifiers) return;
    ...
    if (/[,;\n]/u.test(value)) {
      setReviewingIdentifiers(true); setMessage(undefined);
      const batch = await window.academic.library.reviewScholarlyIdentifiersBatch({ input: value });
      setReviewingIdentifiers(false);
      if (!batch.ok) { setMessage(batch.error.message); return; }
      let added = 0;
      for (const review of batch.value) {
        if (review.entry === undefined) continue;
        const providers = review.provenance.map((item) => item.provider).filter((item, index, all) => all.indexOf(item) === index);
        add('doi', review.entry, [`${review.identifier?.type.toUpperCase() ?? 'Identificador'} resolvido por ${providers.join(', ') || 'provider configurado'}`], review.duplicates.map((duplicate) => ({ referenceId: duplicate.rightId, score: duplicate.score, reasons: duplicate.reasons })));
        added += 1;
      }
      const failures = batch.value.length - added;
      setIdentifier(''); setMessage(`${added} candidato(s) adicionado(s) à inbox.${failures === 0 ? '' : ` ${failures} item(ns) não foram resolvidos; os demais foram preservados.`}`);
      return;
    }
    setReviewingIdentifiers(true);
  ...
  const importFile = async (file: File): Promise<void> => {
    ...
    if (lower.endsWith('.pdf') || file.type === 'application/pdf') {
      const [base64, sha256] = await Promise.all([fileAsBase64(file), fileHash(file)]);
      const reconciled = await window.academic.library.reconcilePdf({ base64 });
  ...
  const confirm = async (item: IntakeItem, existingId?: string): Promise<void> => {
    ...
    const pdf = pendingPdfs.current.get(item.id); if (pdf !== undefined) {
      const role = await requestText('Papel do anexo PDF', 'primary', { description: 'Informe primary, supplementary ou dataset. O papel não é inferido pelo nome do arquivo.' });
      if (role === null) return;
      if (role !== 'primary' && role !== 'supplementary' && role !== 'dataset') { setMessage('Papel do anexo inválido. Use primary, supplementary ou dataset.'); return; }
      const attached = await window.academic.workspace.addAttachment({ referenceId, role, kind: 'file', mediaType: 'application/pdf', name: pdf.name, base64: pdf.base64, displayTitle: pdf.name });
      if (!attached.ok) { setMessage(attached.error.message); return; }
    }
